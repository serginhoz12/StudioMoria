      ...
      <main className="max-w-md mx-auto px-6 -mt-12 relative z-20">
          ...
          <div className="space-y-6 animate-slide-up">
                    ...
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          ...
                          <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                            {isExpired ? 'Encerrada em' : 'Válida até'} {new Date(promo.endDate).toLocaleDateString()}
                          </p>
                       </div>
                    </div>
                    {!isExpired && <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-[8px] font-bold uppercase tracking-widest">Disponível</span>}
                  </div>

                  <p className="text-xs text-gray-600 leading-relaxed italic border-l-2 border-tea-100 pl-4 py-1">"{promo.content}"</p>
                  
                  {linkedSrv && !isExpired && (
                    <div className="pt-4 mt-2 border-t border-gray-50">
                       <p className="text-[8px] text-gray-400 uppercase font-bold tracking-widest mb-3">Vincular Procedimento:</p>
                       <div className="flex justify-between items-center bg-tea-50/50 p-4 rounded-2xl">
                          <span className="text-[10px] font-bold text-tea-900">{linkedSrv.name}</span>
                          <button 
                            onClick={() => { setSelectedService(linkedSrv); setActiveTab('agendar'); setBookingStep(2); }}
                            className="bg-tea-900 text-white px-4 py-2 rounded-xl text-[8px] font-bold uppercase tracking-widest"
                          >
                            Agendar Agora
                          </button>
                       </div>
            ...
            {allMyPromotions.length === 0 && (
              <div className="text-center py-32 bg-gray-50 rounded-[4rem] border-2 border-dashed border-gray-200">
                <p className="text-gray-300 italic font-serif text-lg">Nenhuma oferta recebida ainda.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-6 animate-slide-up">
             {bookingStep === 1 && (
              <div className="space-y-4">
                {services.filter(s => s.isVisible).map(service => {
                  ...
                  return (
                    <div key={service.id} className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm hover:border-tea-200 transition-all flex flex-col h-full relative overflow-hidden group">
                      {hasLinkedPromo && (
                      ...
                      <h4 className="font-bold text-tea-950 text-xl mb-1 group-hover:text-tea-800 transition-colors">{service.name}</h4>
                      <p className="text-gray-400 text-xs mb-6 line-clamp-2 font-light">{service.description}</p>
                      <div className="flex justify-between items-center mt-auto">
                         <div className="text-tea-800 font-bold">R$ {service.price.toFixed(2)}</div>
                         <button onClick={() => { setSelectedService(service); setBookingStep(2); }} className="px-8 py-3 bg-tea-900 text-white rounded-2xl font-bold uppercase text-[9px] tracking-widest hover:bg-black transition-all shadow-md">Selecionar</button>
                      </div>
                    </div>
                  );
                })}
            ...
            {bookingStep === 2 && selectedService && (
              <div className="bg-white rounded-[3.5rem] p-10 shadow-2xl border border-tea-50 space-y-8">
                <div className="flex items-center gap-4">
                   <button onClick={() => setBookingStep(1)} className="p-2 bg-tea-50 rounded-xl hover:bg-tea-100 transition-all">←</button>
                ...
                <div className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-tea-700 uppercase tracking-widest ml-1">Data do Atendimento</label>
                      <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-tea-100 focus:bg-white transition-all" />
                   </div>
                   ...
                   {selectedTime && (
                     <div className="space-y-2 animate-fade-in">
                       <label className="text-[10px] font-bold text-tea-700 uppercase tracking-widest ml-1">Profissional Responsável</label>
                       <select value={selectedProId} onChange={e => setSelectedProId(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-tea-100 transition-all">
                   ...
                   <label className="flex items-start gap-4 p-5 bg-red-50 rounded-3xl cursor-pointer border border-red-100">
                      <input type="checkbox" checked={agreedToCancellation} onChange={e => setAgreedToCancellation(e.target.checked)} className="mt-1 w-4 h-4 rounded text-red-600 border-red-200" />
                ...
                <button disabled={!selectedTime || !selectedProId || !agreedToCancellation} onClick={handleBookSubmit} className="w-full py-6 bg-tea-900 text-white rounded-[2rem] font-bold uppercase text-[11px] tracking-widest shadow-2xl disabled:bg-gray-100 disabled:text-gray-400 transform active:scale-95 transition-all">Solicitar Agendamento</button>
              </div>
            )}
            ...
            {bookingStep === 3 && (
              <div className="text-center py-20 bg-white rounded-[4rem] shadow-2xl px-10 space-y-8 animate-slide-up border border-tea-50">
                ...
                }} className="w-full py-6 bg-tea-900 text-white rounded-3xl font-bold uppercase text-[10px] tracking-[0.2em] shadow-xl hover:bg-black transition-all">📱 Enviar Comprovante</button>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'agenda' && (
          <div className="space-y-6 animate-slide-up">
            ...
            {myBookings.map(b => (
              <div key={b.id} className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-4 hover:border-tea-100 transition-all">
                <div className="flex justify-between items-start">
                   <div>
          ...
          <div className="bg-white p-12 rounded-[4rem] shadow-sm border border-gray-100 text-center space-y-8 animate-slide-up">
             ...
             <div className="pt-6 border-t border-gray-50 space-y-4">
                <div className="flex justify-between items-center text-left">
                   <div>
                     <p className="text-[10px] font-bold text-tea-900 uppercase tracking-widest">Notificações</p>
                     <p className="text-[9px] text-gray-400 uppercase">Alertas via WhatsApp</p>
                   </div>
                   <div className={`w-10 h-5 rounded-full relative transition-all ${customer.receivesNotifications ? 'bg-tea-600' : 'bg-gray-200'}`}>
                      <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${customer.receivesNotifications ? 'left-6' : 'left-1'}`}></div>
                   </div>
                </div>
             </div>
             <button onClick={onLogout} className="w-full py-5 bg-red-50 text-red-600 rounded-2xl font-bold uppercase text-[10px] tracking-widest hover:bg-red-100 transition-all mt-8">Sair da Conta</button>
          </div>
        )}
      </main>
