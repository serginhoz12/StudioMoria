...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ bookings, transactions, customers }) => {
  ...
  const stats = [
    ...
    { label: 'A Receber', value: `R$ ${totalReceivable.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, icon: '💰', color: 'bg-green-50 text-green-600' },
    { label: 'A Pagar', value: `R$ ${totalPayable.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, icon: '💸', color: 'bg-red-50 text-red-600' },
  ];
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-gray-500 text-sm">Acompanhe o desempenho do Studio Moriá.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-lg font-bold text-tea-900">Fluxo Financeiro do Período</h3>
            <div className="text-right">
              <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Saldo Líquido</p>
